<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('applications', function (Blueprint $table) {
            $table->unsignedBigInteger('city_id')->nullable()->change();
            $table->unsignedBigInteger('source_id')->nullable()->change();
            $table->unsignedBigInteger('work_format_id')->nullable()->change();
            $table->unsignedBigInteger('education_id')->nullable()->change();
        });
    }

    public function down(): void
    {
        Schema::table('applications', function (Blueprint $table) {
            $table->unsignedBigInteger('city_id')->nullable(false)->change();
            $table->unsignedBigInteger('source_id')->nullable(false)->change();
            $table->unsignedBigInteger('work_format_id')->nullable(false)->change();
            $table->unsignedBigInteger('education_id')->nullable(false)->change();
        });
    }
};

